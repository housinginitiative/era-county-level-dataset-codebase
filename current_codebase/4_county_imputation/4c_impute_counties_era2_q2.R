# Purpose -----------------------------------------------------------------
# Impute counties for missing addresses for era2_q2
...
era2_q2_deduped <-
  read_parquet(
    str_c(
      ...
      "phpdfs",
      "3_deduplication",
      "era2_q2_deduplicated_2025-03-12.parquet",
      sep = "/"
    )
  )
...
era2_q2_deduped %>% filter(is.na(geocode_county_geoid)) %>% nrow()
# filter: removed 4,560,276 rows (93%), 325,639 rows remaining
...
era2_q2_deduped2 <- era2_q2_deduped %>%
  left_join(single_county_cities, by = "grantee_id_era2")

# See how many counties we are getting back after imputing
era2_q2_deduped2 %>% filter(is.na(imputed_county_geoid)) %>% nrow()
#filter: removed 194,238 rows (4%), 4,691,677 rows remaining
...
era2_q2_deduped3 <- era2_q2_deduped2 %>%
  left_join(
...
era2_q2_deduped3 %>% filter(is.na(imputed_county_geoid)) %>% nrow()
# filter: removed 1,019,838 rows (21%), 3,866,077 rows remaining
...
era2_q2_deduped4 <- era2_q2_deduped3 %>%
...
# See how many counties we are getting back after imputing total
era2_q2_deduped4 %>% filter(is.na(imputed_county_geoid)) %>% nrow()
# filter: removed 3,748,734 rows (77%), 1,137,181 rows remaining
...
era2_q2_deduped5 <- era2_q2_deduped4 %>%
  mutate(
...
# See how many counties we are getting back after imputing total
era2_q2_deduped5 %>% filter(is.na(imputed_county_geoid)) %>% nrow()
#filter: removed 4,411,184 rows (90%), 474,731 rows remaining

# bind back missing counties with full data frame
imputed_counties <- era2_q2_deduped5 %>% 
...
imputed_counties %>%
  ...
  nrow()
#filter: removed 148,249 rows (3%), 4,737,666 rows remaining
# Compared to beginning, we have imputed back ~177k grantees
...
# filter: removed 652,121 rows (13%), 4,233,794 rows remaining
...
# filter: removed 4,859,523 rows (99%), 26,392 rows remaining
...
write_parquet(imputed_counties,
  str_c(
    ...
    "phpdfs",
    "4_county_imputation",
    str_c("era2_q2_imputed_counties_", Sys.Date(),".parquet"),
    sep = "/"
))
