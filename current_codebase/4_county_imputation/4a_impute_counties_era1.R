# Purpose -----------------------------------------------------------------
# Impute counties for missing addresses for ERA1
...
era1_deduped <-
  read_parquet(
    str_c(
      ...
      "phpdfs",
      "3_deduplication",
      "era1_deduplicated_2025-03-06.parquet",
      sep = "/"
    )
  )

# See what we are starting with: 
# 1% of rows are missing a county
era1_deduped %>% filter(is.na(geocode_county_geoid)) %>% nrow()
# filter: removed 7,719,941 rows (99%), 54,831 rows remaining
...
single_county_cities <- grantees_geographies_cities_tracts %>%
  count(grantee_id_era1, imputed_county_geoid) %>%
  count(grantee_id_era1) %>%
  filter(n == 1) %>%
  left_join(grantees_geographies_cities_tracts,
    by = "grantee_id_era1"
  ) %>%
  select(-n) %>%
  select(grantee_id_era1, imputed_county_geoid) %>%
  distinct()
...
era1_deduped2 <- era1_deduped %>%
  left_join(single_county_cities, by = "grantee_id_era1")

# See how many counties we are getting back after imputing
era1_deduped2 %>% filter(is.na(imputed_county_geoid)) %>% nrow()
# filter: removed 485,402 rows (6%), 7,289,370 rows remaining
...
era1_deduped3 %>% filter(is.na(imputed_county_geoid)) %>% nrow()
# filter: removed 1,978,862 rows (25%), 5,795,910 rows remaining
...
# See how many counties we are getting back after imputing total
era1_deduped4 %>% filter(is.na(imputed_county_geoid)) %>% nrow()
# filter: removed 6,183,260 rows (80%), 1,591,512 rows remaining
...
# See how many counties we are getting back after imputing total
era1_deduped5 %>% filter(is.na(imputed_county_geoid)) %>% nrow()
# filter: removed 7,213,741 rows (93%), 561,031 rows remaining

# bind back missing counties with full data frame
imputed_counties <- era1_deduped5 %>% 
...
imputed_counties %>%
  ...
  nrow()
# filter: removed 25,386 rows (<1%), 7,749,386 rows remaining
# Compared to beginning, we have imputed back ~29k grantees

# Out of rows that already had a county, 92% also had county imputed
imputed_counties %>% filter(!is.na(geocode_county_geoid), !is.na(imputed_county_geoid)) %>% nrow()
# filter: removed 590,476 rows (8%), 7,184,296 rows remaining

# Check how many geocoded counties do not match the imputed counties:
# imputed counties do not match geocoded counties for ~114k rows (1%)
imputed_counties %>% filter(geocode_county_geoid!=imputed_county_geoid) %>% nrow()
# filter: removed 7,656,782 rows (98%), 117,990 rows remaining
...
write_parquet(imputed_counties,
  str_c(
    ...
    "phpdfs",
    "4_county_imputation",
    str_c("era1_imputed_counties_", Sys.Date(),".parquet"),
    sep = "/"
))
