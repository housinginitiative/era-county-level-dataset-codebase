# Purpose -----------------------------------------------------------------
# Impute counties for missing addresses for era2_q4
...
era2_q4_deduped <-
  read_parquet(
    str_c(
      ...
      "phpdfs",
      "3_deduplication",
      "era2_q4_deduplicated_2025-03-06.parquet",
      sep = "/"
    )
  )
...
era2_q4_deduped %>% filter(is.na(geocode_county_geoid)) %>% nrow()
# filter: removed 5,634,905 rows (95%), 265,703 rows remaining
...
era2_q4_deduped2 <- era2_q4_deduped %>%
  left_join(single_county_cities, by = "grantee_id_era2")

# See how many counties we are getting back after imputing
era2_q4_deduped2 %>% filter(is.na(imputed_county_geoid)) %>% nrow()
# filter: removed 244,460 rows (4%), 5,656,148 rows remaining
...
era2_q4_deduped3 <- era2_q4_deduped2 %>%
  left_join(
...
era2_q4_deduped3 %>% filter(is.na(imputed_county_geoid)) %>% nrow()
# filter: removed 1,306,913 rows (22%), 4,593,695 rows remaining
...
era2_q4_deduped4 <- era2_q4_deduped3 %>%
...
# See how many counties we are getting back after imputing total
era2_q4_deduped4 %>% filter(is.na(imputed_county_geoid)) %>% nrow()
# filter: removed 4,432,988 rows (75%), 1,467,620 rows remaining
...
era2_q4_deduped5 <- era2_q4_deduped4 %>%
  mutate(
...
# See how many counties we are getting back after imputing total
era2_q4_deduped5 %>% filter(is.na(imputed_county_geoid)) %>% nrow()
# filter: removed 5,262,947 rows (89%), 637,661 rows remaining

# bind back missing counties with full data frame
imputed_counties <- era2_q4_deduped5 %>% 
...
imputed_counties %>%
  ...
  nrow()
# filter: removed 141,915 rows (2%), 5,758,693 rows remaining
# Compared to beginning, we have imputed back ~123k grantees

# Out of rows that already had a county, 87% also had county imputed
imputed_counties %>% filter(!is.na(geocode_county_geoid), !is.na(imputed_county_geoid)) %>% nrow()
# filter: removed 761,449 rows (13%), 5,139,159 rows remaining

# Check how many geocoded counties do not match the imputed counties:
# imputed counties do not match geocoded counties for ~72k rows (1%)
imputed_counties %>% filter(geocode_county_geoid!=imputed_county_geoid) %>% nrow()
# filter: removed 5,825,099 rows (99%), 75,509 rows remaining
...
write_parquet(imputed_counties,
  str_c(
    ...
    "phpdfs",
    "4_county_imputation",
    str_c("era2_q4_imputed_counties_", Sys.Date(),".parquet"),
    sep = "/"
))
