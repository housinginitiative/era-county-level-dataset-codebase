# Purpose -----------------------------------------------------------------
# Impute counties for missing addresses for era2_reachback
...
era2_reachback_deduped <-
  read_parquet(
    str_c(
      ...
      "phpdfs",
      "3_deduplication",
      "era2_reachback_deduplicated_2025-03-12.parquet",
      sep = "/"
    )
  )
...
era2_reachback_deduped %>% filter(is.na(geocode_county_geoid)) %>% nrow()
# filter: removed 6,112,965 rows (98%), 145,963 rows remaining
...
era2_reachback_deduped2 <- era2_reachback_deduped %>%
  left_join(single_county_cities, by = "grantee_id_era2")

# See how many counties we are getting back after imputing
era2_reachback_deduped2 %>% filter(is.na(imputed_county_geoid)) %>% nrow()
# filter: removed 283,847 rows (5%), 5,975,081 rows remaining
...
era2_reachback_deduped3 <- era2_reachback_deduped2 %>%
  left_join(
...
era2_reachback_deduped3 %>% filter(is.na(imputed_county_geoid)) %>% nrow()
# filter: removed 1,456,181 rows (23%), 4,802,747 rows remaining
...
era2_reachback_deduped4 <- era2_reachback_deduped3 %>%
...
# See how many counties we are getting back after imputing total
era2_reachback_deduped4 %>% filter(is.na(imputed_county_geoid)) %>% nrow()
# filter: removed 4,740,621 rows (76%), 1,518,307 rows remaining
...
era2_reachback_deduped5 <- era2_reachback_deduped4 %>%
  mutate(
...
# See how many counties we are getting back after imputing total
era2_reachback_deduped5 %>% filter(is.na(imputed_county_geoid)) %>% nrow()
# filter: removed 5,610,473 rows (90%), 648,455 rows remaining

# bind back missing counties with full data frame
imputed_counties <- era2_reachback_deduped5 %>% 
...
imputed_counties %>%
  ...
  nrow()
# filter: removed 123,711 rows (2%), 6,135,217 rows remaining
...
# filter: removed 670,707 rows (11%), 5,588,221 rows remaining
...
# filter: removed 6,191,900 rows (99%), 67,028 rows remaining
...
write_parquet(imputed_counties,
  str_c(
    ...
    "phpdfs",
    "4_county_imputation",
    str_c("era2_reachback_imputed_counties_", Sys.Date(),".parquet"),
    sep = "/"
))
