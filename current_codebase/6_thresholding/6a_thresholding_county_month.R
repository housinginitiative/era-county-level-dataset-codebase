...
grantees_thresholds_cross_program <- grantees_thresholds_collated %>%  
  ...
  mutate(threshold_variable_goodness = 
           case_when(
             threshold_variable_goodness_era1 & threshold_variable_goodness_era2 ~ 
                       TRUE,
...
#filter: removed 36 rows (71%), 15 rows remaining
...
county_dropping <- grantees_thresholds_cross_program %>% 
  ...
  filter(threshold_all_passing == FALSE) 
# filter: removed 170 rows (66%), 87 rows remaining
...
city_dropping <- grantees_thresholds_cross_program %>% 
  ...
  filter(threshold_all_passing == FALSE)
# filter: removed 62 rows (64%), 35 rows remaining
...
city_county_overlap_dropping <- city_county_overlap %>% 
  # filter: removed 87 rows (81%), 20 rows remaining
...
#filter: removed 2 rows (10%), 18 rows remaining
...
county_city_overlap_dropping <- city_county_overlap %>%   
  # filter: removed 82 rows (77%), 25 rows remaining
  ...
  ungroup() %>% 
  # Rescued:
  # Adams County/Douglas County, CO (x Aurora), 
  # Denton County, TX (x Plano/Ft. Worth),
  # LA County, CA (x Santa Clarita),
  # Miami-Date County, FL (x Hialeah)
  filter(county_population_affected > 0.2)
#filter: removed 6 rows (26%), 17 rows remaining
...
# <chr>                   <chr>                 <chr>                            
#   1 ERA-2101122971_SLT-0190 STATE OF NORTH DAKOTA ERA1 fails                       
# 2 ERA-2101070596_SLT-0165 STATE OF ARIZONA      ERA2 fails                       
# 3 ERA-2101112294_SLT-8619 STATE OF INDIANA      ERA2 fails 
...
# ERA1 fails                9        5    14
# ERA2 fails               19        0    19
# Existing data bad quality               23        3    26
# Missing data               57        4    61
# Passes              240       38   278
...
# Fails by itself              114       13   127
# Fails due to enclosing state failing               60        0    60
# Otherwise fails due to overlapping county/city failing               22        0    22
# Passes              158       38   196
...
# FALSE      55.4% (196) 25.5% (13) 51.6% (209)
# TRUE      44.6% (158) 74.5% (38) 48.4% (196)
...
write_csv(grantees_thresholds_final,
          str_c(data_path, "1_intermediates", "phpdfs", "6_thresholding",
                str_c("county_month_selected_grantees_", Sys.Date(),".csv"),
                sep = "/"))
