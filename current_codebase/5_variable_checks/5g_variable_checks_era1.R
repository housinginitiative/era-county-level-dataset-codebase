...
# Deduplicated ERA1
era1_initial <-read_parquet(str_c(data_path, "1_intermediates", 
                              "phpdfs",
                              "4_county_imputation",
...
treasury_aggregates <- read_rds(str_c(data_path, 
                                      ...
                                      "phpdfs", 
                                      "5_variable_checks",
                                      "phpdf_by_program_era1_2025-03-21.rds",
                                      sep = "/"))
...
large_dollar_threshold <- 
  read_rds(str_c(data_path, "1_intermediates", 
                 "phpdfs", 
                 "5_variable_checks",
...
checks_jurisdiction_era1 <- era1_initial %>% 
  ...
  left_join(grantees_states %>% 
              ...
              select(grantee_id_era1, geoid_jurisdiction_state, in_jurisdiction_state),
            by = c("grantee_id_era1", state_geoid_coalesced = "geoid_jurisdiction_state")) %>% 
  left_join(grantees_counties %>% 
              filter(!is.na(grantee_id_era1)) %>% 
              select(grantee_id_era1, geoid_jurisdiction_county, in_jurisdiction_county),
            by = c("grantee_id_era1", county_geoid_coalesced = "geoid_jurisdiction_county")) %>% 
  left_join(grantees_cities_places %>%
              filter(!is.na(grantee_id_era1)) %>%
              select(grantee_id_era1, geoid_jurisdiction_place, in_jurisdiction_city_place),
            by = c("grantee_id_era1", geocode_place_geoid = "geoid_jurisdiction_place")) %>%
  left_join(grantees_cities_mcds %>%
              filter(!is.na(grantee_id_era1)) %>%
              select(grantee_id_era1, geoid_jurisdiction_mcd, in_jurisdiction_city_mcd),
            by = c("grantee_id_era1", geocode_county_subdivision_geoid = "geoid_jurisdiction_mcd")) %>%
  left_join(grantees_cities_tracts %>% 
              filter(!is.na(grantee_id_era1)) %>% 
              select(grantee_id_era1, geoid_jurisdiction_tract, in_jurisdiction_city_tract),
            by = c("grantee_id_era1", geocode_tract_geoid = "geoid_jurisdiction_tract")) %>% 
  left_join(city_county_crosswalk %>%
              filter(!is.na(grantee_id_era1)) %>%
              select(grantee_id_era1, county_geoid, in_jurisdiction_city_imputed_county),
            by = c("grantee_id_era1", imputed_county_geoid = "county_geoid")) %>%
...
checks_address_era1 <- era1_initial %>% 
  # Raw addresses
...
checks_nonaddress_era1 <- era1_initial %>% 
  # Payee type
  ...
  mutate(check_date_of_payment =
           case_when(is.na(date_of_payment) ~ "No value",
                     date_of_payment < "2021-01-01" ~ "Incorrectly early",
                     date_of_payment > "2022-12-31" ~ "Incorrectly late",
                     .default = "Nominal"),
         .before = original_filename) %>% 
...
checks_all_era1 <- era1_initial %>% 
  left_join(checks_jurisdiction_era1 %>% 
              select(file_specific_row_id, contains("check_")),
            by = "file_specific_row_id") %>% 
  left_join(checks_address_era1 %>% 
              select(file_specific_row_id, contains("check_")),
            by = "file_specific_row_id") %>% 
  left_join(checks_nonaddress_era1 %>% 
              select(file_specific_row_id, contains("check_")),
            by = "file_specific_row_id")

# Row-level goodness labels ------------------------------------------------------------

checks_labeled_era1 <- checks_all_era1 %>% 
...
# Summary by column
summary_variable_era1 <- summarize_by_column_all(checks_all_era1)

# Summary by scenarios
summary_scenarios_era1 <- summarize_by_row_grantees(checks_labeled_era1)

# Percentage payments for rental assistance
summary_rent_percent <- era1_initial %>% 
...
summary_allocation_percent <- era1_initial %>% 
  # filter: 
  filter(amount_of_payment > 0) %>% 
  summarize(.by = c(grantee_id_era1, grantee_id_combined, grantee_name),
            sum_spent = sum(amount_of_payment)) %>% 
  left_join(allocations %>% 
              select(grantee_id_era1 = era_application_number,
...
# Joined
summary_by_grantee_era1 <- summary_scenarios_era1 %>% 
  ...
  left_join(grantees_combined %>% select(grantee_id_era1, grantee_id_combined), by = "grantee_id_combined") %>% 
  left_join(allocations %>% 
              select(grantee_id_era1 = era_application_number,
                     total_updated_allocation),
            by = "grantee_id_era1") %>% 
...
write_parquet(checks_labeled_era1,
              str_c(data_path, "1_intermediates", "phpdfs", "5_variable_checks",
                    str_c("era1_with_variable_checks_", Sys.Date(),".parquet"),
                    sep = "/"))

# Variable-level variable checks summary
write_csv(summary_variable_era1,
          str_c(data_path, "1_intermediates", "phpdfs", "5_variable_checks",
                str_c("era1_variable_checks_summary_", Sys.Date(),".csv"),
                sep = "/"))

# Row-level variable checks summary
write_csv(summary_by_grantee_era1,
          str_c(data_path, "1_intermediates", "phpdfs", "5_variable_checks",
                str_c("era1_grantee_goodness_checks_summary_", Sys.Date(),".csv"),
                sep = "/"))
