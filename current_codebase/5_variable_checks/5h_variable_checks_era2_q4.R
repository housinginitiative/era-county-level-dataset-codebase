...
# Deduplicated ERA2_q4
era2_q4_initial <- read_parquet(str_c(data_path, "1_intermediates", 
                                   "phpdfs",
                                   "4_county_imputation",
...
treasury_aggregates <- read_rds(str_c(data_path, 
                                      ...
                                      "phpdfs", 
                                      "5_variable_checks",
                                      "phpdf_by_program_era2_q4_2025-03-21.rds",
                                      sep = "/"))
...
checks_jurisdiction_era2_q4 <- era2_q4_initial %>% 
...
checks_address_era2_q4 <- era2_q4_initial %>% 
  # Raw addresses
...
checks_nonaddress_era2_q4 <- era2_q4_initial %>% 
  # Payee type
...
checks_all_era2_q4 <- era2_q4_initial %>% 
  left_join(checks_jurisdiction_era2_q4 %>% 
              select(file_specific_row_id, contains("check_")),
            by = "file_specific_row_id") %>% 
  left_join(checks_address_era2_q4 %>% 
              select(file_specific_row_id, contains("check_")),
            by = "file_specific_row_id") %>% 
  left_join(checks_nonaddress_era2_q4 %>% 
              select(file_specific_row_id, contains("check_")),
            by = "file_specific_row_id")

# Row-level goodness labels ------------------------------------------------------------

checks_labeled_era2_q4 <- checks_all_era2_q4 %>% 
...
# Summary by column
summary_variable_era2_q4 <- summarize_by_column_all(checks_all_era2_q4)

# Summary by scenarios
summary_scenarios_era2_q4 <- summarize_by_row_grantees(checks_labeled_era2_q4)

# Percentage payments for rental assistance
summary_rent_percent <- era2_q4_initial %>% 
...
summary_allocation_percent <- era2_q4_initial %>% 
  # filter: 
...
# Joined
summary_by_grantee_era2_q4 <- summary_scenarios_era2_q4 %>% 
...
write_parquet(checks_labeled_era2_q4,
              str_c(data_path, "1_intermediates", "phpdfs", "5_variable_checks",
                    str_c("era2_q4_with_variable_checks_", Sys.Date(),".parquet"),
                    sep = "/"))

# Variable-level variable checks summary
write_csv(summary_variable_era2_q4,
          str_c(data_path, "1_intermediates", "phpdfs", "5_variable_checks",
                str_c("era2_q4_variable_checks_summary_", Sys.Date(),".csv"),
                sep = "/"))

# Row-level variable checks summary
write_csv(summary_by_grantee_era2_q4,
          str_c(data_path, "1_intermediates", "phpdfs", "5_variable_checks",
                str_c("era2_q4_grantee_goodness_checks_summary_", Sys.Date(),".csv"),
                sep = "/"))
