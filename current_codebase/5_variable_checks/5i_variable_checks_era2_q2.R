...
# Deduplicated ERA2_Q2
era2_q2_initial <- read_parquet(str_c(data_path, "1_intermediates", 
                                   "phpdfs",
                                   "4_county_imputation",
                                   "era2_q2_imputed_counties_2025-03-21.parquet",
                                   sep = "/"))
...
treasury_aggregates <- read_rds(str_c(data_path, 
                                      ...
                                      "phpdfs", 
                                      "5_variable_checks",
                                      "phpdf_by_program_era2_q2_2025-03-21.rds",
                                      sep = "/"))
...
checks_jurisdiction_era2_q2 <- era2_q2_initial %>% 
...
checks_address_era2_q2 <- era2_q2_initial %>% 
  # Raw addresses
...
checks_nonaddress_era2_q2 <- era2_q2_initial %>% 
  # Payee type
  ...
  mutate(check_date_of_payment =
           case_when(is.na(date_of_payment) ~ "No value",
                     ...
                     date_of_payment > "2023-06-30" ~ "Incorrectly late",
                     .default = "Nominal"),
         .before = original_filename) %>% 
...
checks_all_era2_q2 <- era2_q2_initial %>% 
  left_join(checks_jurisdiction_era2_q2 %>% 
              select(file_specific_row_id, contains("check_")),
            by = "file_specific_row_id") %>% 
  left_join(checks_address_era2_q2 %>% 
              select(file_specific_row_id, contains("check_")),
            by = "file_specific_row_id") %>% 
  left_join(checks_nonaddress_era2_q2 %>% 
              select(file_specific_row_id, contains("check_")),
            by = "file_specific_row_id")

# Row-level goodness labels ------------------------------------------------------------

checks_labeled_era2_q2 <- checks_all_era2_q2 %>% 
...
# Summary by column
summary_variable_era2_q2 <- summarize_by_column_all(checks_all_era2_q2)

# Summary by scenarios
summary_scenarios_era2_q2 <- summarize_by_row_grantees(checks_labeled_era2_q2)

# Percentage payments for rental assistance
summary_rent_percent <- era2_q2_initial %>% 
...
summary_allocation_percent <- era2_q2_initial %>% 
  # filter: 
...
# Joined
summary_by_grantee_era2_q2 <- summary_scenarios_era2_q2 %>% 
...
write_parquet(checks_labeled_era2_q2,
              str_c(data_path, "1_intermediates", "phpdfs", "5_variable_checks",
                    str_c("era2_q2_with_variable_checks_", Sys.Date(),".parquet"),
                    sep = "/"))

# Variable-level variable checks summary
write_csv(summary_variable_era2_q2,
          str_c(data_path, "1_intermediates", "phpdfs", "5_variable_checks",
                str_c("era2_q2_variable_checks_summary_", Sys.Date(),".csv"),
                sep = "/"))

# Row-level variable checks summary
write_csv(summary_by_grantee_era2_q2,
          str_c(data_path, "1_intermediates", "phpdfs", "5_variable_checks",
                str_c("era2_q2_grantee_goodness_checks_summary_", Sys.Date(),".csv"),
                sep = "/"))
