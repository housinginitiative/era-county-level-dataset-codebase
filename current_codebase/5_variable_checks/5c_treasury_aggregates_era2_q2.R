...
#### LOAD DATA ####
phpdf_ERA2_Q2_cleaned <- read_parquet(
  str_c(
    data_path,
    "1_intermediates", 
    "phpdfs", 
...
era2_q2_expenditures_by_program <- read_excel(
  str_c(
    data_path,
    "0_raw_data", 
    "public_data",
...
era2_q2_expenditures_by_program_cleaned <-  era2_q2_expenditures_by_program %>% 
  rename(program = 1, 
...
# Create era2_q2_grantees_w_expenditures
ERA2_Q2_grantees_w_expenditures <- ERA_grantees_combined %>%
  mutate(grantee_type = if_else(grantee_name == "GOVERNMENT OF THE VIRGIN ISLANDS", 
                                "Territorial Government", 
                                grantee_type)) %>% 
  left_join(era2_q2_expenditures_by_program_cleaned,
            by = c("grantee_name",
                   "grantee_type",
                   "grantee_state" = "state"
            )
...
#should only be 3 programs, each not reported in era2_q2 treasury file
ERA2_Q2_grantees_w_expenditures %>% 
...
phpdf_by_program <- phpdf_ERA2_Q2_cleaned %>%
  group_by(program,
           grantee_id_combined,
           grantee_name,
           grantee_type,
  ...
  ungroup() %>%
  left_join(ERA2_Q2_grantees_w_expenditures, by =  "grantee_id_combined") %>%
...
write_rds(
  ...
  str_c(data_path,
        ...
        "phpdfs",
        "5_variable_checks",
        str_c("phpdf_by_program_era2_q2_", Sys.Date(), ".rds"),
        sep = "/"
  ))
