...
# Deduplicated ERA2_Q1
era2_q1_initial <- read_parquet(str_c(data_path, "1_intermediates", 
                                   "phpdfs",
                                   "4_county_imputation",
                                   "era2_q1_imputed_counties_2025-03-21.parquet",
                                   sep = "/"))
...
treasury_aggregates <- read_rds(str_c(data_path, 
                                      ...
                                      "phpdfs", 
                                      "5_variable_checks",
                                      "phpdf_by_program_era2_q1_2025-03-21.rds",
                                      sep = "/"))
...
checks_jurisdiction_era2_q1 <- era2_q1_initial %>% 
...
checks_address_era2_q1 <- era2_q1_initial %>% 
  # Raw addresses
...
checks_nonaddress_era2_q1 <- era2_q1_initial %>% 
  # Payee type
  ...
  mutate(check_date_of_payment =
           case_when(is.na(date_of_payment) ~ "No value",
                     ...
                     date_of_payment > "2023-03-31" ~ "Incorrectly late",
                     .default = "Nominal"),
         .before = original_filename) %>% 
...
checks_all_era2_q1 <- era2_q1_initial %>% 
  left_join(checks_jurisdiction_era2_q1 %>% 
              select(file_specific_row_id, contains("check_")),
            by = "file_specific_row_id") %>% 
  left_join(checks_address_era2_q1 %>% 
              select(file_specific_row_id, contains("check_")),
            by = "file_specific_row_id") %>% 
  left_join(checks_nonaddress_era2_q1 %>% 
              select(file_specific_row_id, contains("check_")),
            by = "file_specific_row_id")

# Row-level goodness labels ------------------------------------------------------------

checks_labeled_era2_q1 <- checks_all_era2_q1 %>% 
...
# Summary by column
summary_variable_era2_q1 <- summarize_by_column_all(checks_all_era2_q1)

# Summary by scenarios
summary_scenarios_era2_q1 <- summarize_by_row_grantees(checks_labeled_era2_q1)

# Percentage payments for rental assistance
summary_rent_percent <- era2_q1_initial %>% 
...
summary_allocation_percent <- era2_q1_initial %>% 
  # filter: 
...
# Joined
summary_by_grantee_era2_q1 <- summary_scenarios_era2_q1 %>% 
...
write_parquet(checks_labeled_era2_q1,
              str_c(data_path, "1_intermediates", "phpdfs", "5_variable_checks",
                    str_c("era2_q1_with_variable_checks_", Sys.Date(),".parquet"),
                    sep = "/"))

# Variable-level variable checks summary
write_csv(summary_variable_era2_q1,
          str_c(data_path, "1_intermediates", "phpdfs", "5_variable_checks",
                str_c("era2_q1_variable_checks_summary_", Sys.Date(),".csv"),
                sep = "/"))

# Row-level variable checks summary
write_csv(summary_by_grantee_era2_q1,
          str_c(data_path, "1_intermediates", "phpdfs", "5_variable_checks",
                str_c("era2_q1_grantee_goodness_checks_summary_", Sys.Date(),".csv"),
                sep = "/"))
