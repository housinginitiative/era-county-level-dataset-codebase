...
geocodes <-
  read_parquet(
    str_c(
      ...
      "phpdfs",
      "1_geocode",
      "era2_reachback_geocodes_all_2025-03-12.parquet",
      sep = "/"
    )
  )
...
step_1 <- step_0 %>%
  ...
  select(-era1_application_number, -era_application_number, -id) %>% 
...
write_csv(grantee_files,
           str_c(data_path, "1_intermediates", "grantees",
                 str_c("era2_reachback_grantee_files_", Sys.Date(),".csv"),
                 sep = "/"))
...
write_csv(grantee_matches,
          str_c(data_path, "1_intermediates", "grantees",
                str_c("era2_reachback_grantees_", Sys.Date(),".csv"),
                sep = "/"))
...
step_7 <- step_6 %>%
  # filter: removed 2 rows (<1%), 6,259,002 rows remaining
...
write_parquet(step_9,
          str_c(data_path, "1_intermediates", "phpdfs", "2_initial_validation",
                str_c("era2_reachback_initial_validation_", Sys.Date(),".parquet"),
                sep = "/"))
