...
step_0 <-
  read_parquet(
    str_c(
      data_path,
      "0_raw_data",
      "phpdf_ERA2_Q2_2023_GEO.parquet",
      sep = "/"
    )
  )
...
step_1 <- step_0 %>% 
  ...
  mutate(original_filename = "phpdf_ERA2_Q2_2023_GEO.csv", 
         .before = everything()) 
...
step_6 <- step_5 %>% 
  ...
  mutate(grantee_id_era2 =
           case_when(grantee_name == "Doña Ana County" ~ "SLT-0138",
                     grantee_name == "Hialeah city, Florida" ~ "SLT-11854",
                     .default = grantee_id_era2)) %>% 
  mutate(grantee_state =
           case_when(grantee_name == "Doña Ana County" ~ "New Mexico",
                     grantee_name == "Hialeah city, Florida" ~ "Florida",
                     .default = grantee_state)) %>% 
  mutate(grantee_type =
           case_when(grantee_name == "Doña Ana County" ~ "Local Government",
...
# # Write out
 write_csv(grantee_files,
            str_c(data_path, "1_intermediates", "grantees",
                  str_c("era2_q2_grantee_files_", Sys.Date(),".csv"),
                  sep = "/"))
...
write_csv(grantee_matches,
          str_c(data_path, "1_intermediates", "grantees",
                str_c("era2_q2_grantees_", Sys.Date(),".csv"),
                sep = "/"))
...
step_7 <- step_6 %>%
  # filter: removed 2 rows (<1%), 4,898,186 rows remaining
  filter(!address_line_1 %in% c("Total", "Powered by: Middlelayers.org")) %>% 
  # Totals rows from Boise, ID
  # filter: removed 5 rows (<1%), 4,898,181 rows remaining
  filter(!file_specific_row_id %in% c(592636, 596646, 599269, 600311, 601466)) %>% 
  # Slipped headers from Ocean County, NJ
  # filter: removed 5 rows (<1%), 4,898,176 rows remaining
  filter(!(grantee_id_era2 == "SLT-0531" & str_detect(address_line_1, "Record the|Report the|5 numeric")))
...
write_parquet(step_8,
              str_c(data_path, "1_intermediates", "phpdfs", "2_initial_validation",
                    str_c("era2_q2_initial_validation_", Sys.Date(),".parquet"),
                    sep = "/"))
