...
era2_initial <-
  read_parquet(
    str_c(
      ...
      "era2_reachback_initial_validation_2025-03-12.parquet",
      sep = "/"
    )
  )
...
#                 str_c("era2_reachback_dupe_ids_", Sys.Date(),".parquet"),
#                 sep = "/"))
# 
# 
# # Dupes across grantees
...
#                 str_c("era2_reachback_dupes_across_grantees_", Sys.Date(),".csv"),
#                 sep = "/"))
# 
# # Dupes within grantee
# write_csv(dupes_within_grantee,
#           str_c(data_path, "1_intermediates", "phpdfs",  "3_deduplication","reports",
#                 str_c("era2_reachback_dupes_within_grantee_", Sys.Date(),".csv"),
#                 sep = "/"))
# 
# # Dupes across files (+ filenames)
# write_csv(dupes_file_issues,
#           str_c(data_path, "1_intermediates", "phpdfs", "3_deduplication","reports",
#                 str_c("era2_reachback_file_dupes_with_filenames_", Sys.Date(),".csv"),
#                 sep = "/"))
# 
# # Dupes within file (conservative)
# write_csv(dupes_within_file_conservative,
#           str_c(data_path, "1_intermediates", "phpdfs",  "3_deduplication","reports",
#                 str_c("era2_reachback_dupes_within_file_conservative_", Sys.Date(),".csv"),
#                 sep = "/"))
...
fix_step_3a <- fix_step_2c %>%
  # filter: removed 24 rows (<1%), 6,258,916 rows remaining
  filter(dupe_size_within_file_conservative == 1)

# Extract rows to be deduped and dedupe
fix_step_3b <- fix_step_2c %>%
  # filter: removed 6,258,916 rows (>99%), 24 rows remaining 
  filter(dupe_size_within_file_conservative > 1) %>%
  # slice_min: removed 12 rows (50%), 12 rows remaining
  slice_min(
...
test_step_3_3 <- dupes_ids %>%
  # filter: removed 6,258,978 rows (>99%), 24 rows remaining
  filter(dupe_size_within_file_conservative > 1) %>%
...
write_parquet(fix_step_4,
          str_c(data_path, "1_intermediates", "phpdfs", "3_deduplication",
                str_c("era2_reachback_deduplicated_", Sys.Date(),".parquet"),
                sep = "/"))
