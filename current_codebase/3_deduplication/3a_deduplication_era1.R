...
era1_initial <-
  read_parquet(
    str_c(
      ...
      "era1_initial_validation_2025-03-06.parquet",
      sep = "/"
    )
  )
...
files <-
  read_csv(
    str_c(
      ...
      "1_intermediates",
      "grantees",
      "era1_grantee_files_2025-03-06.csv",
      sep = "/"
    )
  )
...
dupes_ids <- era1_initial %>%
  ...
  mutate(
    dupe_size_within_source = n_distinct(grantee_id_era1),
...
dupes_across_grantees_summary <- dupes_across_grantees %>%
  ...
  summarize(n = n()) %>%
  group_by(grantee_id_era1, grantee_name, grantee_state, grantee_type) %>%
...
dupes_file_issues <- dupes_within_grantee %>%
  left_join(files %>%
    select(grantee_id_era1, contains("file_number")), by = "grantee_id_era1") %>%
  filter(dupe_size_1 != 1 | is.na(dupe_size_1))
...
#                 str_c("era1_dupe_ids_", Sys.Date(),".parquet"),
#                 sep = "/"))
...
#                 str_c("era1_dupes_across_grantees_", Sys.Date(),".csv"),
#                 sep = "/"))
#
# # Dupes within grantee
# write_csv(dupes_within_grantee,
#           str_c(data_path, "1_intermediates", "phpdfs",  "3_deduplication","reports",
#                 str_c("era1_dupes_within_grantee_", Sys.Date(),".csv"),
#                 sep = "/"))
#
# # Dupes across files (+ filenames)
# write_csv(dupes_file_issues,
#           str_c(data_path, "1_intermediates", "phpdfs",  "3_deduplication","reports",
#                 str_c("era1_file_dupes_with_filenames_", Sys.Date(),".csv"),
#                 sep = "/"))
#
# # Dupes within file (conservative)
# write_csv(dupes_within_file_conservative,
#           str_c(data_path, "1_intermediates", "phpdfs",  "3_deduplication","reports",
#                 str_c("era1_dupes_within_file_conservative_", Sys.Date(),".csv"),
#                 sep = "/"))
...
fix_step_1 <- dupes_ids %>%
  ...
  # filter: removed 52,004 rows (<1%), 13,344,102 rows remaining
  filter(!(
    grantee_id_era1 == "ERA-2101111681" &
      dupe_size_within_source > 1
  )) %>%
  # Delete records attributed to Mercer County but are actually duped with Mecklenburg County
  # filter: removed 2,325 rows (<1%), 13,341,777 rows remaining
  filter(!(
    grantee_id_era1 == "ERA-2101111968" &
      dupe_size_within_source > 1
  )) %>%
  # Delete records attributed to Cleveland County, OK but are actually duped with Cleveland, OH
  # filter: removed 4,462 rows (<1%), 13,337,315 rows remaining
  filter(!(
    grantee_id_era1 == "ERA-2101112230" &
      dupe_size_within_source > 1
  )) %>%
  # Delete records attributed to Knox County but are actually duped with Tennessee
  # filter: removed 55,947 rows (<1%), 13,281,368 rows remaining
  filter(!(
    grantee_id_era1 == "ERA-2101112008" &
      dupe_size_within_source > 1
  )) %>%
  # New Jersey -> Morris County
  # filter: removed 192 rows (<1%), 13,281,176 rows remaining
  filter(!(
    grantee_id_era1 == "ERA-2101111965" &
      dupe_size_within_source > 1
  )) %>%
  # Allegheny County -> Pittsburgh
  # filter: removed 46 rows (<1%), 13,281,130 rows remaining
  filter(!(
    grantee_id_era1 == "ERA-2101060047" &
      dupe_size_within_source > 1
  )) %>%
  # El Paso County -> El Paso City
  # filter: removed 40 rows (<1%), 13,281,090 rows remaining
  filter(!(
    grantee_id_era1 == "ERA-2101112540" &
      dupe_size_within_source > 1
  )) %>%
  # Maryland -> Frederick County
  # filter: removed 9 rows (<1%), 13,281,081 rows remaining
  filter(!(
    grantee_id_era1 == "ERA-2101111878" &
      dupe_size_within_source > 1
  )) %>%
  # Oklahoma -> Tulsa County, Tulsa City, Oklahoma City,
  # filter: removed 6 rows (<1%), 13,281,075 rows remaining
  filter(!(
    grantee_id_era1 == "ERA-2101112234" &
      dupe_size_within_source > 1
  )) %>%
  # Harris County -> Houston
  # filter: removed 140 rows (<1%), 13,280,935 rows remaining
  filter(!(
    grantee_id_era1 == "ERA-2101060088" &
      dupe_size_within_source > 1
  )) %>%
  # Colorado -> Colorado Springs
  # filter: removed 2 rows (<1%), 13,280,933 rows remaining
  filter(!(
    grantee_id_era1 == "ERA-2101060213" &
      dupe_size_within_source > 1
  )) %>%
  # Pima County -> Tucson
  # filter: removed 4 rows (<1%), 13,280,929 rows remaining
  filter(!(
    grantee_id_era1 == "ERA-2101060180" &
      dupe_size_within_source > 1
  )) %>%
  # Dane County -> Madison
  # filter: removed 2 rows (<1%), 13,280,927 rows remaining
  filter(!(
    grantee_id_era1 == "ERA-2101060055" & dupe_size_within_source > 1
  ))
...
fix_step_2a <- fix_step_1 %>%
  # filter: removed 10,765,406 rows (81%), 2,515,521 rows remaining
  filter(dupe_size_within_grantee == 1)

# Extract rows to be deduped and dedupe
fix_step_2b <- fix_step_1 %>%
  # filter: removed 2,515,521 rows (19%), 10,765,406 rows remaining
  filter(dupe_size_within_grantee > 1) %>%
  left_join(file_sizes %>%
    select(-n), by = c("grantee_id_era1", "file")) %>%
  # slice_min: removed 5,495,320 rows (51%), 5,270,086 rows remaining
  slice_min(
    order_by = size_rank,
...
test_step_2_2 <- fix_step_1 %>%
  ...
  rowwise() %>%
  mutate(expected_n = sum(dupe_size_2 / 2, dupe_size_3 / 3, dupe_size_4 / 4, na.rm = TRUE)) %>%
  ungroup()
...
fix_step_3a <- fix_step_2c %>%
  # filter: removed 6,283 rows (<1%), 7,779,324 rows remaining
  filter(dupe_size_within_file_conservative == 1)

# Extract rows to be deduped and dedupe
fix_step_3b <- fix_step_2c %>%
  # filter: removed 7,779,324 rows (>99%), 6,283 rows remaining
  filter(dupe_size_within_file_conservative > 1) %>%
  # slice_min: removed 3,484 rows (55%), 2,799 rows remaining
  slice_min(
...
test_step_3_2 <- dupes_ids %>%
  filter(grantee_id_era1 == "ERA-2101070387") %>%
  filter(
    file == "Alachua County, Florida - ERA1 Final Report 2022 - participanthouseholdpaymentdata.csv"
  ) %>%
  filter(dupe_size_within_file_conservative > 1) %>%
  mutate(
...
file_dupes_remaining <- fix_step_3c %>%
  group_by(grantee_id_era1, grantee_name, file) %>%
  summarize(n = n()) %>%
  mutate(percent = n / sum(n)) %>%
  ungroup() %>%
  filter(percent != 1)
...
file_dupes_address <- fix_step_3c %>%
  filter(
    grantee_id_era1 %in% file_dupes_remaining$grantee_id_era1 &
      file %in% file_dupes_remaining$file
  ) %>%
  group_by(grantee_id_era1, address_line_1) %>%
...
file_dupes_address_summary <- file_dupes_address %>%
  group_by(grantee_id_era1, grantee_name, file, dupe_size_address) %>%
  summarize(n = n()) %>%
  mutate(percent = n / sum(n)) %>%
  ungroup()
...
file_dupes_address_amount <- fix_step_3c %>%
  ...
  group_by(grantee_id_era1, address_line_1, amount_of_payment) %>%
...
file_dupes_address_amount_summary <- file_dupes_address_amount %>%
  group_by(grantee_id_era1, grantee_name, file, dupe_size_address_amount) %>%
  summarize(n = n()) %>%
  mutate(percent = n / sum(n)) %>%
  ungroup()
...
file_dupes_grantee_check <- file_dupes_address_amount %>%
  filter(grantee_id_era1 == "ERA-2101112311" &
...
# issue is Nashville, where the type_of_assistance field in one file is slightly
# but systematically different from the other file but are clearly the same payments.
# The other grantees' remaining file dupes do not seem to be due to other
# unaddressed sources of systematic duplication.
# We drop the smaller Nashville file, whose addresses are 100% also found in their
# larger file.
fix_step_4 <- fix_step_3c %>%
  # filter: removed 7,351 rows (<1%), 7,774,769 rows remaining
  # 2.28: filter: removed 7,351 rows (<1%), 7,774,772 rows remaining
  filter(
    !(
      grantee_id_era1 == "ERA-2101112311" &
        file == "template_7_individuals_final report.csv"
    )
  ) %>%
  select(-matches("dupe"))
...
nrow(era1_initial) - nrow(fix_step_4)
# 5621334

(nrow(era1_initial) - nrow(fix_step_4)) / nrow(era1_initial)
# 0.4196245
...
#                 "phpdfs",
#                 "3_deduplication",
#                 str_c("era1_deduplicated_", Sys.Date(),".parquet"),
#                 sep = "/"))

toc()
# 2087.471 sec elapsed
