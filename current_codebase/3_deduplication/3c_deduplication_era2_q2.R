...
era2_initial <-
  read_parquet(
    str_c(
      ...
      "era2_q2_initial_validation_2025-03-12.parquet",
      sep = "/"
    )
  )
...
#                 str_c("era2_q2_dupe_ids_", Sys.Date(),".parquet"),
#                 sep = "/"))
# 
# 
# # Dupes across grantees
...
#                 str_c("era2_q2_dupes_across_grantees_", Sys.Date(),".csv"),
#                 sep = "/"))
# 
# # Dupes within grantee
# write_csv(dupes_within_grantee,
#           str_c(data_path, "1_intermediates", "phpdfs",  "3_deduplication","reports",
#                 str_c("era2_q2_dupes_within_grantee_", Sys.Date(),".csv"),
#                 sep = "/"))
# 
# # Dupes across files (+ filenames)
# write_csv(dupes_file_issues,
#           str_c(data_path, "1_intermediates", "phpdfs", "3_deduplication","reports",
#                 str_c("era2_q2_file_dupes_with_filenames_", Sys.Date(),".csv"),
#                 sep = "/"))
# 
# # Dupes within file (conservative)
# write_csv(dupes_within_file_conservative,
#           str_c(data_path, "1_intermediates", "phpdfs",  "3_deduplication","reports",
#                 str_c("era2_q2_dupes_within_file_conservative_", Sys.Date(),".csv"),
#                 sep = "/"))
...
fix_step_1 <- dupes_ids %>% 
  # Riverside County -> Riverside City
  # filter: removed 316 rows (<1%), 4,897,860 rows remaining
  filter(!(grantee_id_era2 == "SLT-2001" & dupe_size_within_source > 1)) %>% 
  # AZ -> Pinal County, Yavapai County
  # filter: removed 49 rows (<1%), 4,897,811 rows remaining
  filter(!(grantee_id_era2 == "SLT-0165" & dupe_size_within_source > 1)) %>% 
  # Ramsey County -> St.Paul
  # filter: removed 11 rows (<1%), 4,897,800 rows remaining
  filter(!(grantee_id_era2 == "SLT-0220" & dupe_size_within_source > 1)) %>% 
  # OK -> Oklahoma County
  # filter: removed one row (<1%), 4,897,799 rows remaining
...
fix_step_2a <- fix_step_1 %>%
  # filter: removed 14,148 rows (<1%), 4,883,651 rows remaining
  filter(dupe_size_within_grantee == 1)

# Extract rows to be deduped and dedupe
fix_step_2b <- fix_step_1 %>%
  # filter: removed 4,883,651 rows (>99%), 14,148 rows remaining
  filter(dupe_size_within_grantee > 1) %>%
  left_join(file_sizes %>%
    select(-n), by = c("grantee_id_era2", "file")) %>%
  # slice_min: removed 7,074 rows (50%), 7,074 rows remaining
  slice_min(
    order_by = size_rank,
...
fix_step_3a <- fix_step_2c %>%
  # filter: removed 7,385 rows (<1%), 4,883,340 rows remaining
  filter(dupe_size_within_file_conservative == 1)

# Extract rows to be deduped and dedupe
fix_step_3b <- fix_step_2c %>%
  # filter: removed 4,883,340 rows (>99%), 7,385 rows remaining
  filter(dupe_size_within_file_conservative > 1) %>%
  # slice_min: removed 4,690 rows (64%), 2,695 rows remaining
  slice_min(
...
test_step_3_3 <- dupes_ids %>%
  # filter: removed 4,890,791 rows (>99%), 7,385 rows remaining
  filter(dupe_size_within_file_conservative > 1) %>%
  # filter: removed 7,367 rows (>99%), 18 rows remaining
...
file_dupes_grantee_check <- file_dupes_address_amount %>%
  filter(grantee_id_era2 == "SLT-0398" & dupe_size_address_amount > 1) %>%
  arrange(address_line_1, file)

# Of the remaining file dupes, only Riverside County has clearly
# remaining dupes (due to data entry issues)
fix_step_4 <- fix_step_3c %>% 
  # filter: removed 120 rows (<1%), 4,885,915 rows remaining
  filter(!(grantee_id_era2 == "SLT-2001" & 
             file == "treasury-q2-2023 - participant data - rivco-erap2-merged.csv")) %>% 
  select(-matches("dupe"))
...
write_parquet(fix_step_4,
          str_c(data_path, "1_intermediates", "phpdfs", "3_deduplication",
                str_c("era2_q2_deduplicated_", Sys.Date(),".parquet"),
                sep = "/"))
