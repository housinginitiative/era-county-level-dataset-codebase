...
era2_initial <-
  read_parquet(
    str_c(
      ...
      "era2_q1_initial_validation_2025-03-12.parquet",
      sep = "/"
    )
  )
...
#                 str_c("era2_q1_dupe_ids_", Sys.Date(),".parquet"),
#                 sep = "/"))
# 
# 
# # Dupes across grantees
...
#                 str_c("era2_q1_dupes_across_grantees_", Sys.Date(),".csv"),
#                 sep = "/"))
# 
# # Dupes within grantee
# write_csv(dupes_within_grantee,
#           str_c(data_path, "1_intermediates", "phpdfs",  "3_deduplication","reports",
#                 str_c("era2_q1_dupes_within_grantee_", Sys.Date(),".csv"),
#                 sep = "/"))
# 
# # Dupes across files (+ filenames)
# write_csv(dupes_file_issues,
#           str_c(data_path, "1_intermediates", "phpdfs", "3_deduplication","reports",
#                 str_c("era2_q1_file_dupes_with_filenames_", Sys.Date(),".csv"),
#                 sep = "/"))
# 
# # Dupes within file (conservative)
# write_csv(dupes_within_file_conservative,
#           str_c(data_path, "1_intermediates", "phpdfs",  "3_deduplication","reports",
#                 str_c("era2_q1_dupes_within_file_conservative_", Sys.Date(),".csv"),
#                 sep = "/"))
...
fix_step_1 <- dupes_ids %>% 
  # AZ -> Pinal County
  # filter: removed 82 rows (<1%), 4,135,233 rows remaining
  filter(!(grantee_id_era2 == "SLT-0165" & dupe_size_within_source > 1)) %>% 
  # WA -> Pierce County
  # filter: removed 23 rows (<1%), 4,135,210 rows remaining
  filter(!(grantee_id_era2 == "SLT-0295" & dupe_size_within_source > 1)) %>% 
  # Ramsey County -> St.Paul
  # filter: removed 11 rows (<1%), 4,135,199 rows remaining
  filter(!(grantee_id_era2 == "SLT-0220" & dupe_size_within_source > 1)) %>% 
  # IL -> Cook County
  # filter: removed 2 rows (<1%), 4,135,197 rows remaining
  filter(!(grantee_id_era2 == "SLT-0135" & dupe_size_within_source > 1)) %>% 
  # Harris County -> Houston
  # filter: removed 2 rows (<1%), 4,135,195 rows remaining
  filter(!(grantee_id_era2 == "SLT-0298" & dupe_size_within_source > 1)) %>% 
  # OK -> Oklahoma County
  # filter: removed one row (<1%), 4,135,194 rows remaining
  filter(!(grantee_id_era2 == "SLT-1423" & dupe_size_within_source > 1))
...
fix_step_2a <- fix_step_1 %>%
  # filter: removed 190,588 rows (5%), 3,944,606 rows remaining
  filter(dupe_size_within_grantee == 1)

# Extract rows to be deduped and dedupe
fix_step_2b <- fix_step_1 %>%
  # filter: removed 3,944,606 rows (95%), 190,588 rows remaining
  filter(dupe_size_within_grantee > 1) %>%
  left_join(file_sizes %>%
    select(-n), by = c("grantee_id_era2", "file")) %>%
  # slice_min: removed 95,294 rows (50%), 95,294 rows remaining
  slice_min(
    order_by = size_rank,
...
test_step_2_2 <- fix_step_1 %>%
  ...
  rowwise() %>%
  mutate(expected_n = sum(dupe_size_2 / 2, na.rm = TRUE)) %>%
  ungroup()
...
fix_step_3a <- fix_step_2c %>%
  # filter: removed 4,751 rows (<1%), 4,035,149 rows remaining
  filter(dupe_size_within_file_conservative == 1)

# Extract rows to be deduped and dedupe
fix_step_3b <- fix_step_2c %>%
  # filter: removed 4,035,149 rows (>99%), 4,751 rows remaining
  filter(dupe_size_within_file_conservative > 1) %>%
  # slice_min: removed 2,754 rows (58%), 1,997 rows remaining
  slice_min(
...
test_step_3_3 <- dupes_ids %>%
  # filter: removed 4,130,564 rows (>99%), 4,751 rows remaining
  filter(dupe_size_within_file_conservative > 1) %>%
...
file_dupes_grantee_check <- file_dupes_address_amount %>%
  filter(grantee_id_era2 == "SLT-2001" & dupe_size_address_amount > 1) %>%
  arrange(address_line_1, file)

# Washington County, OR had some remaining across-file dupes where one file had
# start dates and the other did not.
# Riverside County, CA had one file duped because of a unit number formatting issue.
fix_step_4 <- fix_step_3c %>% 
  # filter: removed 147 rows (<1%), 4,036,999 rows remaining
  filter(!(grantee_id_era2 == "SLT-0215" & 
             file == "Washington County, Oregon - ERA 2 Quarter 1 2023 - 2023-q1 final -template_7_participant_household_payment_data_file_template_3_31_2022 (5).csv")) %>% 
  # filter: removed one row (<1%), 4,036,998 rows remaining
  filter(!(grantee_id_era2 == "SLT-2001" & 
             file == "COUNTY OF RIVERSIDE - ERA 2 Quarter 1 2023 - treasury-q1-2023 - participant data - rivco-erap2-merged.csv")) %>% 
  select(-matches("dupe"))
...
write_parquet(fix_step_4,
          str_c(data_path, "1_intermediates", "phpdfs", "3_deduplication",
                str_c("era2_q1_deduplicated_", Sys.Date(),".parquet"),
                sep = "/"))
