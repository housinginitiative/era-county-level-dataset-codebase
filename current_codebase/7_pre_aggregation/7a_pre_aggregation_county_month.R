...
era_combined_filtered <- era_combined_all %>%
  ...
  # filter: removed 3,707,365 rows (27%), 10,236,484 rows remaining
  # Geography:
  # Drop records without county assignment
  filter(!is.na(county_geoid_coalesced)) %>%
  # filter:removed 11,783 rows (<1%), 10,224,701 rows remaining
  ...
  # filter: filter: removed 1,141,886 rows (11%), 9,082,815 rows remaining
  ...
  # filter: removed 9,856 rows (<1%), 9,072,959 rows remaining
  # Payment amount:
  ...
  # filter: removed 8,760 rows (<1%), 9,064,199 rows remaining
  # Drop negative payments
  filter(amount_of_payment > 0) %>%
  # filter: removed 72,195 rows (1%), 8,992,004 rows remaining
  # Drop large payments
  filter(amount_of_payment < payment_quantile_999) %>%
  # filter: removed 301 rows (<1%), 8,991,703 rows remaining
  # Payment date:
  # Drop records missing payment date
  filter(!is.na(date_of_payment)) %>%
  # filter: removed 35,030 rows (<1%), 8,956,673 rows remaining
  # Drop records too early to be possible (ERA1)
  mutate(check_date_of_payment = 
           case_when(program == "ERA 1" & date_of_payment >= "2021-01-01" & date_of_payment <= "2022-12-31" ~ "Good",
                     source == "ERA2 2023 Q4" & date_of_payment >= "2021-03-01" & date_of_payment <= "2023-12-31" ~ "Good",
                     source == "ERA2 2023 Q2" & date_of_payment >= "2021-03-01" & date_of_payment <= "2023-06-30" ~ "Good",
                     source == "ERA2 2023 Q1" & date_of_payment >= "2021-03-01" & date_of_payment <= "2023-03-31" ~ "Good",
                     source == "ERA2 2023 reachback" & date_of_payment >= "2021-03-01" & date_of_payment <= "2023-12-31" ~ "Good",
                     .default = "Bad"
           )) %>%
  filter(check_date_of_payment == "Good") %>%
  # filter: removed 22,970 rows (<1%), 8,933,703 rows remaining
  filter(date_of_payment <= "2023-03-31") %>% 
  # filter:removed 237,020 rows (3%), 8,696,683 rows remaining
...
final_pre_aggregation <- era_combined_filtered %>%
  ...
  ungroup() %>%
  # Month of payment
  mutate(month_of_payment = floor_date(date_of_payment, unit = "months")) %>%
...
write_parquet(
  ...
  str_c(data_path, "1_intermediates", "phpdfs", "7_pre_aggregation",
    str_c("county_month_data_ready_for_aggregation_", Sys.Date(), ".parquet"),
    sep = "/"
  )
)
...
write_csv(
  ...
  str_c(data_path, "1_intermediates", "phpdfs", "7_pre_aggregation",
        str_c("county_month_missing_address_summary_", Sys.Date(), ".csv"),
        sep = "/"
  )
)
